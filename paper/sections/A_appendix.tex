...
\todo{URL}. \todo{datasheet, croissant metadata, hosting}
...
\input{sections/_reference_table}
...
\paragraph{A stronger form of irreversibility is implemented and not enabled.} Commitment is irreversible throughout, and \S\ref{sec:regime} shows what it costs an
agent that the other three conditions do not. Beyond it the environment implements damage: fruit abortion above
$T_{\mathrm{CRIT}}$ puts a unit into a recovery period graded by how far the threshold was
exceeded, so that a badly chosen treatment costs capacity in the \emph{following} round and
not only the units it occupied. All four tasks set the recovery length to zero, with the
comment ``0 = off (default, keeps existing results unchanged)''---which is the reasoning
this appendix opens by arguing against, and we record it as such rather than defend it.

It is not dead code for want of triggering. Across the reference strategies, $55\%$ to
$60\%$ of committed units exceed the threshold, by $4.3$ to $4.6\,^\circ$C on average, so at
$\texttt{damage\_scale} = 6$ a sixty-day recovery period would take roughly two fifths of
the facility out of the next round. Enabling it is the first item in the next version. The
same parameter is also the natural dial for the ablation that would turn ``the four
conditions matter jointly'' from a claim about the domain into a measurement: running one
task at recovery lengths of zero, sixty and two hundred and ten days prices irreversibility
directly, against agents that are otherwise identical.
