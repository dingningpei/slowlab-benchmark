...
\begin{enumerate}[leftmargin=1.5em,itemsep=3pt,topsep=3pt]
  ...
  \item We conduct \emph{the first evaluation of language-model agents under these four
    conditions jointly}: five models, four tasks, twenty instances each, run with and
    without standard tools. Over $800$ episodes no arm reaches the regret a perfect
    reasoner would carry having run nothing at all. We expected over-exploration and found
    the reverse: the median design replicates two distinct treatments across the facility
    whatever the task needs, leaving $99\%$ of six-factor screens unable to separate the
    effects they were commissioned to separate, and the recommendations then use a
    twentieth of what even those designs did support. Supplying a screening tool repairs the
    designs and makes the campaigns end \emph{worse} in sixteen of twenty cells: one
    intervention, two measures, opposite directions.
\end{enumerate}
