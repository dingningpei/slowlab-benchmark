...
They still rank agents on realised regret, and they are the tasks on which every agent in
\S\ref{sec:results}, scripted and otherwise, converts a few per cent of what its design
carried --- the two facts are the same fact. The cause is not noise we chose. It is that
...
$13.2\times$. Against the weaker reference of a model's own zero-shot recommendation,
...
\textsc{Sanity} and \textsc{Optimise} they rank-correlate at $-0.80$ and $-0.60$ within
...
careless designs. All but four of the $1137$ submitted declare a randomisation seed,
replication is high, and the allocation respects the control hierarchy; they are designs
...
$+0.80$ on \textsc{Optimise} and $-0.80$ on \textsc{Transfer}: the model with the best
design is the one that reads it worst there. A single score would have reported only their
product, and \S\ref{sec:results:findings} shows an intervention that moves them in opposite
directions on purpose.

\subsubsection{Standard tools improve the design and damage the answer}
\label{sec:results:findings}

\input{sections/_tools_table}
